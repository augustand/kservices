...
def run_local(log="debug", env="local", port="8010"):
    """
    运行本地项目
    fab run_local:"log='debug',env='dev',port='8010'"
    ...
    local(
        ...
        --log-level="{log}" \
        -e "app_env={env}" \
        --worker-class="egg:meinheld#gunicorn_worker" wacai.main:app
        """.format(port=port, log=log, env=env)
    )


def _clone(p_name='wacai'):
